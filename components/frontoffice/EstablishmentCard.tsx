...
const EstablishmentCard = memo(function EstablishmentCard({
  ...
  return (
    <article
      ...
      className={`group relative bg-white rounded-2xl shadow-card-luxury transition-all duration-500 overflow-hidden border border-gray-100 cursor-pointer ${
    ...
    >
      {/* Image Container */}
      <div className="relative h-[20rem] overflow-hidden bg-gray-200">
        {!imageLoaded && (
        ...
        <div className={`absolute bottom-4 right-4 flex space-x-2 transition-all duration-300 ${
          ...
          >
            <svg className="w-5 h-5 text-luxury-text" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <div className="p-6">
        ...
        <div className="mb-4">
          <div className="flex items-start justify-between mb-2">
            <h3 className="text-xl font-bold text-[hsl(var(--color-luxury-dark))] group-hover:text-[hsl(var(--color-luxury-gold))] transition-colors">
              {name}
            </h3>
            <div className="flex items-center space-x-1 text-luxury-text">
              {renderStars(rating)}
            </div>
          </div>
          ...
          <div className="flex items-center text-sm text-luxury-text">
            <span className="font-semibold text-luxury-gold">{rating}</span>
        ...
        <div className="mb-6">
          <div className="flex flex-wrap gap-2">
            {amenities.slice(0, 4).map((amenity, index) => (
              <span
                key={index}
                className="px-2 py-1 bg-[hsl(var(--color-luxury-gold-light))]/10 text-luxury-text text-xs rounded-full border border-luxury-gold-light font-medium"
              >
                {amenity}
              </span>
            ))}
        ...
        <div className="flex space-x-3">
          <button
            type="button"
            onClick={handleViewDetails}
            className="flex-1 px-4 py-2.5 border-2 border-[hsl(var(--color-luxury-text))]/20  text-luxury-text rounded-xl hover:border-[hsl(var(--color-luxury-gold-light))] transition-all duration-200 font-medium text-sm focus:outline-none focus:ring-4 focus:ring-amber-500/50 touch-manipulation min-h-[44px]"
          ...
          <button
            ...
            className={`flex-1 px-4 py-2.5 rounded-xl font-medium text-sm transition-all duration-200 touch-manipulation min-h-[44px] ${
              isAvailable
                ? 'bg-gradient-luxury text-luxury-cream shadow-luxury transform hover:scale-105'
                : 'bg-luxury-cream text-luxury-text cursor-not-allowed'
            }`}
      ...
      {/* Hover Effect Border */}
      <div className={`absolute inset-0 rounded-2xl border border-luxury-gold-light transition-opacity duration-300 ${
