      ...
      <div className="mb-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-luxury-text mt-2">{user.email}</p>
          </div>
          <div className="flex gap-3">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg flex items-center gap-2 ${
                user.isActive 
                  ? 'bg-yellow-600 text-luxury-cream hover:bg-yellow-700' 
                  : 'bg-green-600 text-luxury-cream hover:bg-green-700'
              }`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h2 className="text-xl font-bold text-luxury-dark mb-4">Informations personnelles</h2>
            
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-gray-500">Prénom</label>
                  <p className="text-luxury-dark">{user.firstName}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Nom</label>
                  <p className="text-luxury-dark">{user.lastName}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-luxury-dark">{user.email}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Téléphone</label>
                  <p className="text-luxury-dark">{user.phone || 'N/A'}</p>
                </div>
              </div>

              <div>
                <label className="text-sm font-medium text-gray-500">Établissement</label>
                <p className="text-luxury-dark">{user.establishmentId?.name || 'Aucun'}</p>
              </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              <div className="p-4 bg-blue-50 rounded-lg">
                <div className="text-sm text-luxury-text mb-2">Rôle</div>
                {getRoleBadge(user.role)}
              </div>

              <div className="p-4 bg-green-50 rounded-lg">
                <div className="text-sm text-luxury-text mb-2">Statut du compte</div>
          ...
          {user.permissions && user.permissions.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h2 className="text-xl font-bold text-luxury-dark mb-4">Permissions</h2>
