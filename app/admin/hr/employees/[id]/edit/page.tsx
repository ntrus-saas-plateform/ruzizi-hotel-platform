...
import type { EmployeeResponse, UpdateEmployeeInput } from '@/types/employee.types';
...
export default function EditEmployeePage() {
    ...
    const fetchEmployee = async () => {
        try {
            ...
            setFormData({
                ...
                dateOfBirth: new Date(employeeData.personalInfo.dateOfBirth).toISOString().split('T')[0],
                gender: employeeData.personalInfo.gender,
                nationality: employeeData.personalInfo.nationality,
                idNumber: employeeData.personalInfo.idNumber,
                ...
                establishmentId: employeeData.employmentInfo.establishmentId,
                hireDate: new Date(employeeData.employmentInfo.hireDate).toISOString().split('T')[0],
                contractType: employeeData.employmentInfo.contractType,
                salary: employeeData.employmentInfo.salary.toString(),
                status: employeeData.employmentInfo.status,
            });
        } catch (err) {
            console.error('Employee fetch error:', err);
            setError(err instanceof Error ? err.message : 'Erreur lors du chargement de l\'employé');
        } finally {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Prepare data according to UpdateEmployeeInput interface
            const submitData: UpdateEmployeeInput = {
                personalInfo: {
        ...
        return (
            <div className="p-6 max-w-6xl mx-auto">
                <div className="text-center">
                    ...
                    <button
                        onClick={() => router.back()}
                        className="mt-4 text-luxury-gold hover:text-luxury-dark"
                    >
                        ← Retour
                    </button>
                </div>
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto">
            <div className="mb-6">
                ...
                <h1 className="text-3xl font-bold text-luxury-dark">Modifier l'Employé</h1>
                <p className="text-luxury-text mt-2">
                    Modifier le profil de {employee.personalInfo.firstName} {employee.personalInfo.lastName}
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {employee.employmentInfo.employeeNumber && (
                            <div className="md:col-span-2">
                                ...
                                <input
                                    type="text"
                                    value={employee.employmentInfo.employeeNumber}
                ...
                <div className="flex gap-4 pt-4">
                    ...
                    >
                        {loading ? 'Mise à jour...' : 'Mettre à jour l\'employé'}
                    </button>
                </div>
            </form>
        </div>
