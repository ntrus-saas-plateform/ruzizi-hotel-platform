...
/**
 * GET /api/bookings/walkin/by-date
...
export async function GET(request: NextRequest) {
  return requireAuth(async (req) => {
    try {
      ...
      const bookings = await BookingService.getWalkInBookingsByDate(accommodationId, date);

      return createSuccessResponse({ bookings, date: dateStr, accommodationId });
    } catch (error) {
